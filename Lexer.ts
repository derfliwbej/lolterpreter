...
export class Lexer {
    ...
    private static keywords: Map<string, string>;

    constructor(source: string) {
        this.source = source;

        Lexer.keywords = new Map<string, string>();
    ...
    public scan(): void {
        ...
        var firstChar: string = this.advance();
        ...
        if (firstChar == '\"') {
            ...
            if (type == null) {
                ...
                if (this.isLiteral(lexeme)) {
                    ...
                    if(this.match("WIN", lexeme)) value = true;
                    else if(this.match("FAIL", lexeme)) value = false;
                    else if(this.match("((-?[1-9][0-9]*)|0)", lexeme)) value = parseInt(lexeme);
                    else value = parseFloat(lexeme);
                } else if (this.isIdentifier(lexeme)) type = Token.VARIABLE_IDENTIFIER;
                else {
            ...
            if(type === Lexer.keywords.get("BTW")) {
                ...
                this.ignoreLine();
            } else this.tokens.push(new Token(this.line, lexeme, type, value));
        }
    ...
    private match(pattern: string, s: string) {
    ...
    private isTokenDelimiter(char_pos: number): boolean {
        if(char_pos >= this.source.length) return true;
        return this.source.charAt(char_pos).trim() === "";
    }

    private isPartOfTwo(lexeme: string): boolean {
    ...
    private isPartOfThree(lexeme: string): boolean {
    ...
    private isLiteral(lexeme: string): boolean {
    ...
    private isIdentifier(lexeme: string): boolean {
        if (this.match("(^|\\s)([a-zA-Z][a-zA-Z0-9_]*)($|\\s)", lexeme)) return true;
        return false;
    }
